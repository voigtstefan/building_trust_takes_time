...
summary_statistics <- orderbooks %>% 
  left_join(exchange_characteristics %>% 
              select(exchange, region, taker_fee, btc_withdrawal_fee, 
                     no_of_confirmations, margin_trading, business_accounts, rating_categorial), by = "exchange") %>%
...
summary_statistics <- summary_statistics |>
  select(exchange, n, spread, spread_bp, taker_fee, btc_withdrawal_fee, no_of_confirmations, margin_trading, business_accounts, region, rating_categorial)

## define column labels
colnames(summary_statistics) <- c(" ", "Orderbooks", "Spread (USD)", 
                                  "Spread (bp)", "Taker Fee", "With. Fee",
                                  "Conf. ", "Margin", "Business", "Region", "Rating")

## print to latex
print(xtable(summary_statistics, 
             digits = c(0, 0,0,2,2,2,5,0,0,0,0,0)), 
